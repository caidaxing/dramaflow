...
function getUserId(rawToken: string): number | null {
  const token = (rawToken || "").replace("Bearer ", "");
  try {
...
export default (nsp: Namespace) => {
  nsp.on("connection", async (socket: Socket) => {
    ...
    if (userId == null) {
      console.log("[productionAgent] 连接失败，无法解析用户");
      socket.disconnect();
      return;
    }
    ...
    // 记忆隔离：统一前缀为 userId:projectId:...
    if (!isolationKey.startsWith(`${userId}:`)) {
      isolationKey = `${userId}:${isolationKey}`;
    }
    ...
    socket.on("updateContext", (data: { isolationKey: string; projectId: number; scriptId: number }, callback) => {
      isolationKey = data.isolationKey.startsWith(`${userId}:`) ? data.isolationKey : `${userId}:${data.isolationKey}`;
