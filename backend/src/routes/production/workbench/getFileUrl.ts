...
export default router.post(
    ...
    async (req, res) => {
        ...
        const assetIds = items.filter((item: any) => item.sources == "assets").map((item: any) => item.id)
        if (storyboardIds.length) {
            const rows = await u.db("o_storyboard").whereIn("id", storyboardIds).select("projectId");
            const projectIds = Array.from(new Set(rows.map((r: any) => r.projectId)));
            for (const projectId of projectIds) await u.tenant.assertProjectOwner(u.db, projectId, req.user.id);
        }
        if (assetIds.length) {
            const rows = await u.db("o_assets").whereIn("id", assetIds).select("projectId");
            const projectIds = Array.from(new Set(rows.map((r: any) => r.projectId)));
            for (const projectId of projectIds) await u.tenant.assertProjectOwner(u.db, projectId, req.user.id);
        }
        ...
        if (assetIds.length) {
            const assetsPaths = await u.db("o_assets").leftJoin("o_image", "o_image.id", "o_assets.imageId").whereIn("o_assets.id", assetIds).select("o_assets.id", "o_image.filePath");
