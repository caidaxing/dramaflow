...
export default router.post(
  ...
  async (req, res) => {
    const { id } = req.body;
    const storyboardData = await u.db("o_storyboard").where("id", id).select("id", "track", "trackId", "flowId", "projectId").first();
    if (!storyboardData) return res.status(400).send(error("未找到该分镜"));
    await u.tenant.assertProjectOwner(u.db, storyboardData.projectId, req.user.id);
