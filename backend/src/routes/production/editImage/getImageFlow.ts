...
export default router.post(
  ...
  async (req, res) => {
    ...
    const flowRow = await u.db("o_storyboard").where("flowId", id).select("projectId").first();
    const flowRow2 = flowRow ? null : await u.db("o_assets").where("flowId", id).select("projectId").first();
