...
export default router.post(
  ...
  async (req, res) => {
    ...
    const assets = await u.db("o_assets").where("id", assetsId).select("id", "imageId", "type", "projectId").first();
    if (assets) await u.tenant.assertProjectOwner(u.db, assets.projectId, req.user.id);
