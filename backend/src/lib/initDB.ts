...
export default async (knex: Knex, forceInit: boolean = false): Promise<void> => {
  const tables: TableSchema[] = [
    // 用户表
    {
      ...
      builder: (table) => {
        ...
        table.text("phone");
        table.text("nickname");
        table.text("role").defaultTo("user"); // admin/user
        table.text("status").defaultTo("enabled"); // enabled/disabled
        table.integer("balance").defaultTo(0); // 余额（单位：分）
    ...
    //AI调用用量明细表（Phase 1 计费）
    {
      ...
      builder: (table) => {
        ...
        table.text("type"); // chat/image/video/tts
        table.text("model"); // 模型名
        table.text("vendorId");
        table.integer("inputTokens");
        table.integer("outputTokens");
        table.integer("count"); // 图片张数/条数
        table.integer("duration"); // 视频时长（秒）
        table.integer("cost"); // 成本（分）
        table.integer("charge"); // 向用户扣费（分）
        table.boolean("useUserKey").defaultTo(false); // 是否 BYOK
        table.integer("createTime");
        table.primary(["id"]);
        table.unique(["id"]);
        table.index(["userId"]);
    ...
    //充值流水表（Phase 1 计费）
    {
      ...
      builder: (table) => {
        table.integer("id").notNullable();
        table.integer("userId");
        table.integer("amount"); // 金额（分）
        table.text("method"); // manual/manual_admin 等
        table.integer("operatorId"); // 操作人
        table.text("status"); // success/pending/failed
        table.text("remark");
        table.integer("createTime");
        table.primary(["id"]);
        table.unique(["id"]);
    ...
    //模型单价表（Phase 1 计费）
    {
      ...
      builder: (table) => {
        ...
        table.text("type"); // chat/image/video/tts
        table.text("vendorId");
        table.text("model"); // 模型名，* 或具体
        table.double("pricePerUnit"); // 单价值（分）
        table.text("unit"); // per1kToken/perImage/perSecond/perItem
        table.boolean("enabled").defaultTo(true);
        table.integer("createTime");
        table.integer("updateTime");
        table.primary(["id"]);
        table.unique(["id"]);
        table.unique(["type", "vendorId", "model"]);
      },
      // 示例定价，运营可在后台调整（backend 接口 /api/setting/priceConfig）
      initData: async (knex) => {
        const now = Date.now();
        const base = { enabled: 1, createTime: now, updateTime: now };
        const rows: any[] = [
          // 文本 chat：per1kToken（每千 token 单价，分）
          { type: "chat", vendorId: "deepseek", model: "*", pricePerUnit: 0.3, unit: "per1kToken", ...base },
          { type: "chat", vendorId: "klingai", model: "*", pricePerUnit: 0.5, unit: "per1kToken", ...base },
          { type: "chat", vendorId: "openai", model: "*", pricePerUnit: 1.5, unit: "per1kToken", ...base },
          { type: "chat", vendorId: "toonflow", model: "*", pricePerUnit: 0.8, unit: "per1kToken", ...base },
          { type: "chat", vendorId: "volcengine", model: "*", pricePerUnit: 0.5, unit: "per1kToken", ...base },
          { type: "chat", vendorId: "minimax", model: "*", pricePerUnit: 0.6, unit: "per1kToken", ...base },
          { type: "chat", vendorId: "atlascloud", model: "*", pricePerUnit: 0.8, unit: "per1kToken", ...base },
          // 图片 image：perImage（每张单价，分）
          { type: "image", vendorId: "klingai", model: "*", pricePerUnit: 20, unit: "perImage", ...base },
          { type: "image", vendorId: "vidu", model: "*", pricePerUnit: 18, unit: "perImage", ...base },
          { type: "image", vendorId: "deepseek", model: "*", pricePerUnit: 15, unit: "perImage", ...base },
          { type: "image", vendorId: "volcengine", model: "*", pricePerUnit: 15, unit: "perImage", ...base },
          { type: "image", vendorId: "minimax", model: "*", pricePerUnit: 15, unit: "perImage", ...base },
          // 视频 video：perSecond（每秒单价，分）
          { type: "video", vendorId: "klingai", model: "*", pricePerUnit: 8, unit: "perSecond", ...base },
          { type: "video", vendorId: "volcengine", model: "*", pricePerUnit: 6, unit: "perSecond", ...base },
          { type: "video", vendorId: "vidu", model: "*", pricePerUnit: 7, unit: "perSecond", ...base },
          { type: "video", vendorId: "toonflow", model: "*", pricePerUnit: 7, unit: "perSecond", ...base },
          { type: "video", vendorId: "minimax", model: "*", pricePerUnit: 6, unit: "perSecond", ...base },
          // TTS：perItem（每次/条，分），主流供应商统一 3 分
          { type: "tts", vendorId: "toonflow", model: "*", pricePerUnit: 3, unit: "perItem", ...base },
          { type: "tts", vendorId: "volcengine", model: "*", pricePerUnit: 3, unit: "perItem", ...base },
          { type: "tts", vendorId: "minimax", model: "*", pricePerUnit: 3, unit: "perItem", ...base },
          // 全局兜底价：未单独配置的 (type,vendorId,model) 也有价
          { type: "chat", vendorId: "*", model: "*", pricePerUnit: 0.5, unit: "per1kToken", ...base },
          { type: "image", vendorId: "*", model: "*", pricePerUnit: 15, unit: "perImage", ...base },
          { type: "video", vendorId: "*", model: "*", pricePerUnit: 7, unit: "perSecond", ...base },
          { type: "tts", vendorId: "*", model: "*", pricePerUnit: 3, unit: "perItem", ...base },
        ];
        await knex("o_price").insert(rows);
      },
    },
    //站点配置表（Phase 2 品牌化/白标）：key-value 结构，存 siteName/logoUrl/primaryColor/icpNo/customerService/registerOpen/announcement
    {
      name: "o_site_config",
      builder: (table) => {
        table.integer("id").notNullable();
      ...
      initData: async (knex) => {
        ...
        const defaults: { key: string; value: string }[] = [
          { key: "siteName", value: "ToonFlow" },
          { key: "logoUrl", value: "" },
          { key: "primaryColor", value: "#0052D9" },
          { key: "icpNo", value: "" },
          { key: "customerService", value: "" },
          { key: "registerOpen", value: "1" },
          { key: "announcement", value: "" },
        ];
        await knex("o_site_config").insert(
          defaults.map((d, i) => ({ id: now + i, key: d.key, value: d.value, updateTime: now })),
        );
      },
    },
  ];
  ...
  // Phase 2 品牌化：环境变量预置（白标部署用 docker -e 注入）。
  // initDB 在表创建后执行，若对应环境变量存在则覆盖 o_site_config 的默认值。
  const siteEnvMap: Record<string, string> = {
    SITE_NAME: "siteName",
    LOGO_URL: "logoUrl",
  ...
  for (const [envKey, configKey] of Object.entries(siteEnvMap)) {
    const val = process.env[envKey];
    if (val != null && val !== "") {
      const existing = await knex("o_site_config").where("key", configKey).first();
      if (existing) {
        await knex("o_site_config").where("key", configKey).update({ value: val, updateTime: Date.now() });
        console.log(`[初始化数据库] 环境变量覆盖站点配置: ${configKey}=${val}`);
      } else {
        await knex("o_site_config").insert({ id: Date.now(), key: configKey, value: val, updateTime: Date.now() });
      }
    }
  }
};
