...
import { hashPassword, isBcryptHash } from "@/utils/auth";
...
export default async (knex: Knex): Promise<void> => {
  ...
  // Phase 1 多租户/计费：o_user 扩展字段（仅加列，不破坏现有列）
  await addColumn("o_user", "email", "text");
  await addColumn("o_user", "phone", "text");
  await addColumn("o_user", "nickname", "text");
  await addColumn("o_user", "role", "text");
  await addColumn("o_user", "status", "text");
  await addColumn("o_user", "balance", "integer");
  await addColumn("o_user", "createTime", "integer");

  // 为新增列填充默认值（SQLite ALTER TABLE ADD COLUMN 不支持默认值，需在迁移时回填）
  await db("o_user").whereNull("role").update({ role: "user" });
  await db("o_user").whereNull("status").update({ status: "enabled" });
  await db("o_user").whereNull("balance").update({ balance: 0 });
  await db("o_user").whereNull("createTime").update({ createTime: Date.now() });
  // 内置管理员（原 seed 用户 id=1 / name=admin）设为 admin 角色
  await db("o_user").where("name", "admin").update({ role: "admin" });

  // Phase 1 安全：存量明文密码迁移为 bcrypt 哈希
  // 读取全部用户，凡 password 非 bcrypt 格式（$2 开头）则视为明文，哈希后回写。
  const users = await db("o_user").select("id", "password");
  let migratedCount = 0;
  for (const user of users) {
    if (user.password != null && !isBcryptHash(user.password)) {
      const hash = await hashPassword(user.password);
      await db("o_user").where("id", user.id).update({ password: hash });
      migratedCount++;
    }
  }
  if (migratedCount > 0) {
    console.log(`[数据库迁移] 已将 ${migratedCount} 个用户的明文密码迁移为 bcrypt 哈希`);
  }
  ...
  // Phase 1 多租户：o_tasks 加 userId，用于任务归属隔离
  await addColumn("o_tasks", "userId", "integer");
  // 存量任务回填 userId：优先通过 o_project.userId 反查，查不到填 0
  const taskRows = await db("o_tasks").select("id", "projectId", "userId");
  for (const task of taskRows) {
    if (task.userId != null) continue;
    let userId = 0;
    if (task.projectId != null) {
      const project = await db("o_project").where("id", task.projectId).select("userId").first();
      if (project && project.userId != null) userId = project.userId;
    }
    await db("o_tasks").where("id", task.id).update({ userId });
  }
