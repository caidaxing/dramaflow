<template>
  <t-card :title="'#' + (activeTrackIndex + 1) + $t('workbench.generate.videoMenu')" header-bordered style="height: 100%">
    <template #actions>
      <estimate-price ref="estimateRef" type="video" :model="modelParmas.model" :params="{ duration: modelParmas.duration }" />
...
const estimateRef = ref<{ checkBalance: () => Promise<boolean> } | null>(null);
/** 供父组件在发起生成前做余额校验：余额充足返回 true */
function checkBalance(): Promise<boolean> {
  return estimateRef.value?.checkBalance() ?? Promise.resolve(true);
}
